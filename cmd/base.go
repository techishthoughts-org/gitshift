...
func (c *BaseCommand) addCommonFlags(cmd *cobra.Command) {
	// Add common flags that all commands might need
...
func (c *BaseCommand) RequireArgs(cmd *cobra.Command, min, max int) error {
	...
	if len(args) < min {
		return errors.New(errors.ErrCodeInvalidInput,
			fmt.Sprintf("command requires at least %d arguments, got %d", min, len(args))).
			WithContext("required_min", min).
			WithContext("required_max", max).
			WithContext("provided", len(args))
	}

	if max > 0 && len(args) > max {
		return errors.New(errors.ErrCodeInvalidInput,
			fmt.Sprintf("command accepts at most %d arguments, got %d", max, len(args))).
			WithContext("required_min", min).
			WithContext("required_max", max).
			WithContext("provided", len(args))
	}
...
// RequireExactArgs ensures the command has exactly the specified number of arguments
func (c *BaseCommand) RequireExactArgs(cmd *cobra.Command, count int) error {
	args := cmd.Flags().Args()

	if len(args) != count {
		return errors.New(errors.ErrCodeInvalidInput,
			fmt.Sprintf("command requires exactly %d arguments, got %d", count, len(args))).
			WithContext("required", count).
			WithContext("provided", len(args))
	}
...
func (c *BaseCommand) PrintError(ctx context.Context, message string, err error, fields ...observability.Field) {
	c.logger.Error(ctx, message, append(fields, observability.F("error", err.Error()))...)
	fmt.Printf("❌ %s: %v\n", message, err)
}

// PrintWarning prints a warning message
func (c *BaseCommand) PrintWarning(ctx context.Context, message string, fields ...observability.Field) {
	c.logger.Warn(ctx, message, fields...)
	fmt.Printf("⚠️  %s\n", message)
}
...
// PrintDebug prints a debug message (only in verbose mode)
func (c *BaseCommand) PrintDebug(ctx context.Context, message string, fields ...observability.Field) {
	if c.GetFlagBool(nil, "verbose") {
